<template>
  <modal :show="show" @close="closeModal()" :closeable="false" max-width="3xl">
    <template #title>
      <h3 class="text-lg font-medium capitalize text-gray-900" v-if="host">{{ `${host.first_name} ${host.last_name}` }}'s Properties</h3>
    </template>
    <template #content>
      <div class="px-6 py-4">
...
const fetchHostProperties = () => {
  axios.get(route('properties.index', {host: host.value.id})).then(response => {
...
const showModal = (args) => {
  host.value = args?.host ? args.host : null
...
onMounted(() => {
  emitter.on('showHostProperties', showModal)
})


onUnmounted(() => {
  emitter.off('showHostProperties', showModal)
})
